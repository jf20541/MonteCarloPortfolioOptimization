...
class PortfolioOptimization:
    ...
    def monte_carlo(self, n_iterations):
        ...
        returns, volatility, sharperatio = np.zeros(n_iterations), np.zeros(n_iterations), np.zeros(n_iterations)
        ...
        montecarlos_df = pd.DataFrame(data=[returns, volatility, sharperatio, portfolio_weights]).T.infer_objects()
        ...
        best_weights = montecarlos_df.loc[montecarlos_df['Sharpe Ratio'].idxmax()]['Portfolio Weights']
